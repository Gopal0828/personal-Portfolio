...
function Navbar() {
    ...
    {mobileNav && (
        <div className="lg:hidden flex items-center cursor-pointer">
            <div>
                {isSidebarOpen && (
                    <ul className="absolute right-4 top-16 bg-gray-900 bg-opacity-95 p-4 rounded-lg shadow-2xl flex flex-col items-center gap-y-4 border border-gray-700 w-64">
                        {["Experience", "Skills", "Projects", "Education", "Contact"].map((item) => (
                            <li key={item} className="mobile-navbar-list">
                                <Link
                                    href={item === "Contact" ? "./src/contact.html" : "#"}
                                    className="block text-gray-300 hover:text-blue-500 transition-colors duration-300 ease-in-out"
                                >
                                    {item}
                                </Link>
                            </li>
